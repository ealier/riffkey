-- Импорт в облачную MySQL (db4free.net, TiDB Cloud и т.п.)
-- В phpMyAdmin выберите СВОЮ базу → вкладка «Импорт» → этот файл.
-- Строки CREATE DATABASE / USE здесь нет — их на бесплатном хостинге обычно нельзя выполнять.
